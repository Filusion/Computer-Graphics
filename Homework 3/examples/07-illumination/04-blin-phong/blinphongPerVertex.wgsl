...
fn vertex(input: VertexInput) -> VertexOutput {
    ...
    let cosTheta = dot(normalize(vec3(0, 0, -1)), -L);
    let cutoff = smoothstep(cos(light.beamWidth), 1.0, cosTheta);
    let directivity = pow(cosTheta, light.directivity) * cutoff;

    let blinnPhong = pow(max(dot(N, H), 0.0), material.shininess) * material.specular;

    let diffuseLight = lambert * attenuation * light.color * directivity;
    let specularLight = blinnPhong * attenuation * light.color * directivity;
    ...
    output.specularLight = blinnPhong * attenuation * light.color;
